...
<div class="product-catalog row checkout-page">
    <div class="col-md-12 checkout">
        ...
        <!-- payment address -->
        <div class="row" id="payment-address">
            <div class="col-md-12">
                <div class="checkout-heading">
                    <span>
                        <?php 
                        if (!$logged) { 
                            echo $text_checkout_account; 
                        } else {
                            echo $text_checkout_payment_address;
                        }
                        ?>
                    </span>
                </div>
        ...
        <div class="row" id="shipping-method">
            <div class="col-md-12">
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (json['redirect']) {
                ...
                $('#checkout .checkout-content').prepend(print_alert(json['error']['warning']));
            }
        },
        error: function(xhr, ajaxOptions, thrownError) {
            alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
...
$('#button-guest').live('click', function() {
    ...
    $.ajax({
        ...
        type: 'post',
        data: $('#payment-address input[type=\'text\'], #payment-address input[type=\'email\'], #payment-address input[type=\'checkbox\']:checked, #payment-address input[type=\'radio\']:checked, #payment-address input[type=\'hidden\'], #payment-address select, #payment-address textarea'),
        dataType: 'json',
        beforeSend: function() {
        ...
        success: function(json) {
            $('.warning, .error, .temp_message').remove();
            if (json['redirect']) {
                location = json['redirect'];
            } else if (json['error']) {
                if (json['error']['warning']) {
                    $('#payment-address .checkout-content').prepend(print_alert(json['error']['warning']));
                }

                if (json['error']['firstname']) {
                    error_message += json['error']['firstname']+'<br/>';
                    //$('#payment-address input[name=\'firstname\'] + br').after('<span class="error">' + json['error']['firstname'] + '</span>');
                }

                if (json['error']['lastname']) {
                    error_message += json['error']['lastname']+'<br/>';
                    //$('#payment-address input[name=\'lastname\'] + br').after('<span class="error">' + json['error']['lastname'] + '</span>');
                }
                ...
                if (json['error']['company_id']) {
                    error_message += json['error']['company_id']+'<br/>';
                    //$('#payment-address input[name=\'company_id\'] + br').after('<span class="error">' + json['error']['company_id'] + '</span>');
                }

                if (json['error']['tax_id']) {
                    error_message += json['error']['tax_id']+'<br/>';
                    //$('#payment-address input[name=\'tax_id\'] + br').after('<span class="error">' + json['error']['tax_id'] + '</span>');
                }
                ...
                if (json['error']['country']) {
                    error_message += json['error']['country']+'<br/>';
                    //$('#payment-address select[name=\'country_id\'] + br').after('<span class="error">' + json['error']['country'] + '</span>');
                }

                if (json['error']['zone']) {
                    error_message += json['error']['zone']+'<br/>';
                    //$('#payment-address select[name=\'zone_id\'] + br').after('<span class="error">' + json['error']['zone'] + '</span>');
                }
                if (error_message != '') {
                    $('#payment-address .checkout-content').prepend(print_alert(error_message,'danger'));
                }
            } else {
                <?php if ($shipping_required) { ?>
                var shipping_address = $('#payment-address input[name=\'shipping_address\']:checked').attr('value');
...
$('#button-guest-shipping').live('click', function() {
    ...
    $.ajax({
        ...
        type: 'post',
        data: $('#shipping-address input[type=\'text\'], #shipping-address input[type=\'hidden\'], #shipping-address select, #shipping-address textarea'),
        dataType: 'json',
        beforeSend: function() {
        ...
        success: function(json) {
            ...
            if (json['redirect']) {
                ...
                if (json['error']['warning']) {
                    $('#shipping-address .checkout-content').prepend(print_alert(json['error']['warning']));
                }
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (json['redirect']) {
                ...
                if (json['error']['warning']) {
                    $('#shipping-method .checkout-content').prepend(print_alert(json['error']['warning']));
                }
            } else {
                $.ajax({
                    url: 'index.php?route=checkout/payment_method',
    ...
    $.ajax({
        ...
        success: function(json) {
            ...
            if (json['redirect']) {
                ...
                if (json['error']['warning']) {
                    $('#payment-method .checkout-content').prepend(print_alert(json['error']['warning']));
                }
            } else {
                $.ajax({
...
function print_alert(message,type) {
    var type = (!type) ? 'warning' : type;
    var ret = '<div class="row temp_message">';
        ...
        ret += '<span class="sr-only">Close</span>';
        ret += '</button>';
        ret += message;
        ret += '</div>';
        ret += '</div>';
